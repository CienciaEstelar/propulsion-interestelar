...
Vela láser, 1g a $0.2c$ & $\sim2\times10^{15}$ J & 0.01$\times$ diario \\
Vela láser, 1t a $0.2c$ & $\sim2\times10^{21}$ J & $\sim10\times$ anual \\
...
La Figura~\ref{fig:sincronizacion} muestra que incluso con un reloj atómico de iones de última generación (DSAC, $\sigma_A=10^{-15}$), el error de sincronización acumulado al final del crucero a $\alpha$ Centauri es de $\sim1.4\times10^8$ ms ($\sim4.2\times10^{13}$ km en distancia), dominado por la dilatación temporal relativista. Esto subraya la necesidad de correcciones relativistas continuas durante toda la misión.
...
El cálculo de energía total ($2\times10^{15}$ J) corresponde a la energía cinética final. Para obtener la energía eléctrica requerida: $\eta_{\text{total}} = \eta_{\text{eléctrica}} \times \eta_{\text{láser}} \times \eta_{\text{acoplamiento}}$. Asumiendo $\eta_{\text{láser}} = 0.1\%$, $\eta_{\text{acoplamiento}} \approx 20\%$, y $\eta_{\text{eléctrica}} \approx 90\%$, se obtiene $\eta_{\text{total}} \approx 1.8\times10^{-4}$. Por tanto, $E_{\text{eléctrica}} \approx 1.1\times10^{19}$ J, equivalente a $\sim$6\% de la producción eléctrica humana anual.
...
Bajo iluminación de 100 GW sobre una vela de 4 m$^2$, el flujo es de 25 MW/m$^2$. Con reflectividad 99.99\%, la potencia absorbida es $\sim$2.5 kW/m$^2$. La temperatura de equilibrio: $T = (P_{\text{abs}}/(\sigma\cdot\varepsilon))^{1/4}$. Para $\varepsilon = 0.1$, $T \approx 3000$ K (cercano al punto de fusión del SiC). Para $\varepsilon = 0.5$, $T \approx 1500$ K (manejable). El control de la emisividad infrarroja es crítico.
